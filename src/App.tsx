...
export default function App() {
  ...
  async function carregarDados(usuarioAtual = user) {
    ...
    setNotificacoes(notifSnap.docs.map((d) => ({ id: d.id, ...d.data() })));
  }
  ...
  async function enviarInforme() {
    try {
      ...
      await criarNotificacao(clienteDestino, "Novo informativo", "Você recebeu um novo informativo no portal.");
  ...
  const isContador = user.email === EMAIL_CONTADOR;
  const empresa = isContador ? "Pitágoras Contabilidade" : clientes.find((c) => c.email === user.email)?.nomeEmpresa || user.email;
  ...
  const cnds = documentosDoCliente.filter((d) => ["CND Federal", "CND Estadual", "CND Municipal", "CND FGTS"].includes(d.departamento));
  const documentosRecebidos = documentosDoCliente.filter((d) => d.tipo === "contador_enviou" && !["Informes", "Boletos de Honorários", "CND Federal", "CND Estadual", "CND Municipal", "CND FGTS"].includes(d.departamento));
  const notificacoesUsuario = notificacoes.filter((n) => n.destino === user.email);
  const notificacoesNaoLidas = notificacoesUsuario.filter((n) => !n.lida).length;

  const documentosContador = docs.filter((d) => {
  ...
  const totalBoletos = isContador ? docs.filter((d) => d.departamento === "Boletos de Honorários").length : boletos.length;
  ...
  const NotificacoesCard = () => (
    <div style={styles.bigCard}>
      <h2 style={styles.cardTitle}>Notificações</h2>
      {notificacoesUsuario.length === 0 ? (
        <p style={styles.empty}>Nenhuma notificação.</p>
      ) : (
        notificacoesUsuario.slice().reverse().slice(0, 8).map((item, i) => (
          <div key={i} style={{ ...styles.notificationItem, background: item.lida ? "#fff" : "#f3e8ff" }} onClick={() => marcarNotificacaoLida(item.id)}>
  ...
  return (
    <div style={styles.page}>
      <aside style={styles.sidebar}>
        <div style={styles.logoBox}><img src={logo} style={styles.sideLogo} /></div>
        <div style={styles.menu}>
          <button style={styles.menuActive}>🏠 Dashboard</button>
          {isContador ? (
            categoriasContador.map((item) => (
              <button key={item} onClick={() => setCategoria(item)} style={categoria === item ? styles.menuActive : styles.menuItem}>📁 {item}</button>
            ))
          ) : (
            <>
      ...
      <main style={styles.content}>
        <header style={styles.header}>
          <div>
            <h1 style={styles.welcome}>{isContador ? "Painel do Contador" : `Olá, ${empresa}`}</h1>
            <p style={styles.subWelcome}>{isContador ? "Gerencie clientes, documentos, boletos, CNDs e informativos." : "Bem-vindo ao Portal do Cliente Pitágoras Contabilidade."}</p>
          </div>
          <div style={styles.profileBox}>
            <div style={styles.bell}>🔔{notificacoesNaoLidas > 0 && <div style={styles.badge}>{notificacoesNaoLidas}</div>}</div>
            <div style={styles.profile}><strong>{empresa}</strong><span>{isContador ? "Contador" : "Cliente"}</span></div>
          </div>
        </header>

        <section style={styles.statsGrid}>
          <div style={styles.statCard}><div style={styles.iconPurple}>📁</div><div><p style={styles.statLabel}>Documentos</p><h2 style={styles.statNumber}>{totalDocumentos}</h2><span style={styles.statPurple}>Total de documentos</span></div></div>
          <div style={styles.statCard}><div style={styles.iconBlue}>💳</div><div><p style={styles.statLabel}>Boletos</p><h2 style={styles.statNumber}>{totalBoletos}</h2><span style={styles.statBlue}>Honorários</span></div></div>
          <div style={styles.statCard}><div style={styles.iconGreen}>✅</div><div><p style={styles.statLabel}>CNDs</p><h2 style={styles.statNumber}>{totalCnds}</h2><span style={styles.statGreen}>Certidões disponíveis</span></div></div>
          <div style={styles.statCard}><div style={styles.iconOrange}>⏱</div><div><p style={styles.statLabel}>Pendentes</p><h2 style={styles.statNumber}>{totalPendentes}</h2><span style={styles.statOrange}>Aguardando ação</span></div></div>
        </section>
        ...
        {!isContador ? (
          <section style={styles.mainGrid}>
            <div style={styles.bigCard}>
              ...
              {documentosRecebidos.length === 0 ? <p style={styles.empty}>Nenhum documento recebido.</p> : documentosRecebidos.map((item, i) => (
                <div key={i} style={styles.docItem}>
                  <div><strong>{item.nome}</strong><p style={styles.muted}>{item.departamento}</p></div>
                  {(item.caminho || item.boletoUrl || item.invoiceUrl) && <button style={styles.downloadButton} onClick={() => abrirDocumentoOuLink(item)}>Baixar</button>}
                </div>
              ))}
              <div style={styles.uploadBox}>
                ...
                <select style={styles.input} value={categoria} onChange={(e) => setCategoria(e.target.value)}>
                  <option>Fiscal</option><option>Contábil</option><option>Pessoal</option><option>Contratos</option>
                </select>
                <input style={styles.input} type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} />
                <button style={styles.primaryButton} onClick={enviarDocumento} disabled={enviando}>{enviando ? `Enviando ${uploadProgress}%` : "Enviar Documento"}</button>
                {enviando && <div style={styles.loadingBar}><div style={{ ...styles.loadingProgress, width: `${uploadProgress}%` }} /></div>}
              </div>
            </div>
            ...
            <div style={styles.sideCards}>
              ...
              <div style={styles.bigCard}>
                ...
                {informativos.length === 0 ? <p style={styles.empty}>Nenhum informativo disponível.</p> : informativos.map((item, i) => <div key={i} style={styles.infoItem}>{item.nome}</div>)}
              </div>
              <div style={styles.bigCard}>
                <h2 style={styles.cardTitle}>Boletos de Honorários</h2>
                {boletos.length === 0 ? <p style={styles.empty}>Nenhum boleto disponível.</p> : boletos.map((item, i) => (
                  <div key={i} style={styles.docItem}>
                    <div><strong>{item.nome}</strong>{item.valor && <p style={styles.muted}>R$ {item.valor}</p>}{item.vencimento && <p style={styles.muted}>Vencimento: {item.vencimento}</p>}</div>
                    {(item.boletoUrl || item.invoiceUrl || item.caminho) && <button style={styles.downloadButton} onClick={() => abrirDocumentoOuLink(item)}>Abrir boleto</button>}
                  </div>
                ))}
              </div>
              <div style={styles.bigCard}>
                <h2 style={styles.cardTitle}>CNDs Disponíveis</h2>
                {cnds.length === 0 ? <p style={styles.empty}>Nenhuma CND disponível.</p> : cnds.map((item, i) => (
                  <div key={i} style={styles.docItem}>
                    <div><strong>{item.departamento}</strong><p style={styles.muted}>{item.nome}</p></div>
                    {item.caminho && <button style={styles.downloadButton} onClick={() => abrirDocumentoOuLink(item)}>Abrir</button>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <section style={styles.mainGrid}>
            <div style={styles.bigCard}>
              ...
              <select style={styles.input} value={clienteDestino} onChange={(e) => setClienteDestino(e.target.value)}>
                ...
                {clientes.map((cliente, i) => <option key={i} value={cliente.email}>{cliente.nomeEmpresa} - {cliente.email}</option>)}
              </select>
              <select style={styles.input} value={categoria} onChange={(e) => setCategoria(e.target.value)}>
              ...
              <select style={styles.input} value={filtroCliente} onChange={(e) => setFiltroCliente(e.target.value)}>
                ...
                {clientes.map((cliente, i) => <option key={i} value={cliente.email}>{cliente.nomeEmpresa}</option>)}
              </select>
                              ...
                              <div key={mi} style={styles.monthBlock}>
                                ...
                                {docsMes.map((item: any, di: number) => (
                                  <div key={di} style={styles.docItem}>
                                    <div><strong>{item.nome}</strong><p style={styles.muted}>{item.departamento}</p>{item.valor && <p style={styles.muted}>R$ {item.valor}</p>}{item.vencimento && <p style={styles.muted}>Vencimento: {item.vencimento}</p>}</div>
                                    {(item.caminho || item.boletoUrl || item.invoiceUrl) && <button style={styles.downloadButton} onClick={() => abrirDocumentoOuLink(item)}>Abrir</button>}
                                  </div>
                                ))}
                              </div>
                            );
            ...
            <div style={styles.sideCards}>
              ...
              <div style={styles.bigCard}>
                ...
                <input style={styles.input} placeholder="Valor do boleto. Ex: 150,00" value={valorBoleto} onChange={(e) => setValorBoleto(e.target.value)} />
                <input style={styles.input} type="date" value={vencimentoBoleto} onChange={(e) => setVencimentoBoleto(e.target.value)} />
                <textarea style={{ ...styles.input, height: 90 }} placeholder="Descrição do boleto" value={descricaoBoleto} onChange={(e) => setDescricaoBoleto(e.target.value)} />
                <button style={styles.primaryButton} onClick={emitirBoletoAsaas} disabled={emitindoBoleto}>{emitindoBoleto ? "Emitindo boleto..." : "Emitir Boleto"}</button>
              </div>
              ...
              <div style={styles.bigCard}>
                ...
                {clientes.length === 0 ? <p style={styles.empty}>Nenhum cliente cadastrado.</p> : clientes.map((cliente, i) => (
                  <div key={i} style={styles.docItem}>
                    <div><strong>{cliente.nomeEmpresa}</strong><p style={styles.muted}>{cliente.email}</p><p style={styles.muted}>{cliente.cnpj}</p></div>
                    <button style={styles.downloadButton} onClick={() => setClienteDestino(cliente.email)}>Selecionar</button>
                  </div>
                ))}
              </div>
              ...
              <div style={styles.bigCard}>
                ...
                <textarea style={{ ...styles.input, height: 120 }} placeholder="Digite o informativo" value={informe} onChange={(e) => setInforme(e.target.value)} />
...
const styles: any = {
  page: { minHeight: "100vh", background: "#f8fafc", display: "flex", fontFamily: "Inter, Arial, sans-serif", color: "#0f172a" },
  loginPage: { minHeight: "100vh", background: "#f8fafc", display: "flex", justifyContent: "center", alignItems: "center", fontFamily: "Inter, Arial" },
  loginCard: { width: 430, background: "#fff", padding: 40, borderRadius: 28, textAlign: "center", boxShadow: "0 20px 60px rgba(15,23,42,.10)", border: "1px solid #e5e7eb" },
  loginLogo: { width: 250, marginBottom: 20 },
  loginTitle: { margin: 0, fontSize: 32, color: "#0f172a" },
  loginText: { color: "#64748b", marginBottom: 28 },
  sidebar: { width: 290, background: "#ffffff", borderRight: "1px solid #e5e7eb", minHeight: "100vh", padding: 24, boxSizing: "border-box", display: "flex", flexDirection: "column", position: "sticky", top: 0 },
  logoBox: { textAlign: "center", marginBottom: 35 },
  sideLogo: { width: 190 },
  menu: { display: "flex", flexDirection: "column", gap: 10 },
  menuItem: { border: "none", background: "transparent", padding: "14px 16px", borderRadius: 14, textAlign: "left", fontWeight: 600, color: "#334155", cursor: "pointer", fontSize: 15 },
  menuActive: { border: "none", background: "linear-gradient(135deg,#ede9fe,#f3e8ff)", padding: "14px 16px", borderRadius: 14, textAlign: "left", fontWeight: 700, color: "#6d28d9", cursor: "pointer", fontSize: 15 },
  exitButton: { marginTop: "auto", border: "none", background: "transparent", color: "#ef4444", fontWeight: 700, textAlign: "left", cursor: "pointer", fontSize: 15 },
  ...
  bell: { position: "relative", width: 54, height: 54, background: "#fff", borderRadius: 16, display: "flex", justifyContent: "center", alignItems: "center", boxShadow: "0 10px 30px rgba(15,23,42,.08)" },
  badge: { position: "absolute", top: -5, right: -5, background: "#ef4444", color: "#fff", width: 22, height: 22, borderRadius: 999, display: "flex", justifyContent: "center", alignItems: "center", fontSize: 12, fontWeight: 700 },
  profile: { background: "#fff", padding: "12px 18px", borderRadius: 16, boxShadow: "0 10px 30px rgba(15,23,42,.08)", display: "flex", flexDirection: "column" },
  statsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20, marginBottom: 25 },
  statCard: { background: "#fff", borderRadius: 24, padding: 24, display: "flex", alignItems: "center", gap: 20, boxShadow: "0 12px 35px rgba(15,23,42,.07)", border: "1px solid #e5e7eb" },
  iconPurple: { width: 62, height: 62, borderRadius: 22, background: "#ede9fe", color: "#7c3aed", display: "flex", justifyContent: "center", alignItems: "center", fontSize: 28 },
  iconBlue: { width: 62, height: 62, borderRadius: 22, background: "#dbeafe", color: "#2563eb", display: "flex", justifyContent: "center", alignItems: "center", fontSize: 28 },
  iconGreen: { width: 62, height: 62, borderRadius: 22, background: "#dcfce7", color: "#16a34a", display: "flex", justifyContent: "center", alignItems: "center", fontSize: 28 },
  iconOrange: { width: 62, height: 62, borderRadius: 22, background: "#ffedd5", color: "#f97316", display: "flex", justifyContent: "center", alignItems: "center", fontSize: 28 },
  ...
  bigCard: { background: "#fff", borderRadius: 24, padding: 24, boxShadow: "0 12px 35px rgba(15,23,42,.07)", border: "1px solid #e5e7eb" },
  cardTitle: { marginTop: 0, fontSize: 22, fontWeight: 800 },
  input: { width: "100%", padding: "14px 15px", marginBottom: 14, borderRadius: 14, border: "1px solid #dbe4ee", fontSize: 15, boxSizing: "border-box", outline: "none", background: "#fff" },
  primaryButton: { width: "100%", padding: 15, border: "none", borderRadius: 14, background: "#7c3aed", color: "#fff", fontWeight: 700, cursor: "pointer" },
  downloadButton: { border: "1px solid #ddd6fe", borderRadius: 12, padding: "9px 14px", background: "#fff", color: "#7c3aed", fontWeight: 700, cursor: "pointer" },
  docItem: { background: "#fff", borderBottom: "1px solid #eef2f7", padding: "15px 0", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 15 },
  uploadBox: { marginTop: 25, padding: 22, borderRadius: 20, border: "2px dashed #c4b5fd", background: "#faf5ff" },
  infoItem: { background: "#faf5ff", color: "#4c1d95", borderRadius: 14, padding: 14, marginBottom: 10, fontWeight: 600 },
  ...
  clientBlock: { background: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: 18, padding: 16, marginBottom: 16 },
  ...
  loadingBar: { height: 8, borderRadius: 999, background: "#e9d5ff", overflow: "hidden", marginTop: 12 },
  loadingProgress: { height: "100%", background: "#7c3aed" },
  notificationItem: { padding: 14, borderRadius: 16, marginBottom: 10, border: "1px solid #e9d5ff", cursor: "pointer" },
  notificationText: { margin: "6px 0 0", color: "#64748b", fontSize: 14 },
  unreadText: { display: "inline-block", marginTop: 8, fontSize: 12, color: "#7c3aed", fontWeight: 700 },
};
